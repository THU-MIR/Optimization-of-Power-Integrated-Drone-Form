...
class servo_legEnv_v2(MujocoEnv,utils.EzPickle):
    '''
...
'''
    ...
    @property
    def terminated(self):
        terminated= True if self.target_dist> 100 else False
        terminated= True if self.quat[2] < 0 else False
        terminated=False
        return terminated
    ...
    def _get_obs(self):
        ...
        if self.task=="hover":
            ...
            return np.concatenate((self.position,self.velocity,self.shape_trans,self.thrust,self.distance))
        if self.task=="speed":
            self.distance=np.array([self.position[2]-self.target_position])
            self.vel_dist=self.velocity[0:3]-self.target_vel
            return np.concatenate((self.position,self.velocity,self.shape_trans,self.thrust,self.distance,self.vel_dist))
    ...
    def step(self, actions):
        ...
        action=np.zeros((28))
        self.thrust=actions[:4]
        action=np.append(action,self.thrust)
        ...
        quat=torch.tensor(self.quat).cuda(0)
        velocity=torch.tensor(velocity).cuda(0)
        xyz_position=torch.tensor(xyz_position).cuda(0)
        ...
        if self.task=="hover":
            ...
            target_vel=torch.tensor(self.target_vel).cuda(0)
            target_position=torch.tensor(self.target_position).cuda(0)
            ...
            rewards = vel_reward + vel_reward *(up_reward+ spin_reward)+pos_reward
        ...
        info = {
            ...
            "x_position": xyz_position[0],
            "y_position": xyz_position[1],
            "z_position": xyz_position[2],
